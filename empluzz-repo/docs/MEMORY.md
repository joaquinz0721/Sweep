...
- [ ] **Get the work onto `main`, again.** This was closed on 2026-08-24 in the old repo. It is open again in `joaquinz0721/Sweep`, where `main` holds only the uploaded snapshot (`cd0c8a5`, "Add files via upload") and everything since lives on `claude/freeform-cover-letter-ftpu5i`: the writing system, the tone rules, the coursework fix, the publish runbook and the current build. **That branch is the one to read from until it lands.** It is a clean fast-forward from `main`, but a session does not push to `main` without Joaquin saying so, so it waits on him. A second branch, `claude/publish-app-command-center-merged-m3z4ik`, is merged into it and can be deleted once `main` moves.
  **One trap worth knowing.** A fresh clone's local `main` ref can be stale, and `git log origin/main` reads that stale ref until you `git fetch`. On 2026-08-24 it showed `cd0c8a5 Add files via upload` when the real `origin/main` was seven commits further on at `b7f3071`. **Fetch before you judge how far behind a branch is**, or you will write a false history into this file. That happened and was caught by reading the push output.
