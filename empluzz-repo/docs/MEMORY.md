...
> **STATE AS OF 2026-09-08, after a publish from a cloud Claude Code session.**
> Live version is **`1788869745-d88d`**, carrying **56 internship rows** (41 plus
> the fifteen from the 2026-09-08 sweep), 12 scholarship rows, 20 calendar rows,
> and **28 applied ticks**. `SWEPT` now reads 2026-09-08.
> The ticks: this file used to record 17, measured 2026-08-25. A read on
> 2026-09-08 returned 28, because Joaquin kept ticking and the page republished
> itself each time. That is normal and is not drift. **28 went into the publish
> and 28 came out, state block byte identical.** Always measure against a fresh
> read on the day, never against a number written here.
> Route 0a is now **proven four times**. This publish landed on the first try by
> following `docs/artifact-publish-runbook.md` literally: fetch, full `Read` of
> all 1109 lines, rebuild, verify, publish, with nothing in between.
