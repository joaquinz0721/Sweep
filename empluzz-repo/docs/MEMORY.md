...
4. **THE ROUTE THAT WORKS FOR UNATTENDED WORK: a cloud routine. PROVEN 2026-08-22, no longer a hypothesis.** A one-shot routine firing a fresh unattended cloud session in the `Sweep` environment read the live tracker (123,859 bytes, read only) and published a real change to a second artifact that declares the same `artifact` capability, clearing the same baseVersion guard. Full evidence and the two remaining caveats are in `docs/artifact-write-routes.md`. **This is what fixes the sweeps**, and the design that has them refuse to publish can now be revisited. Minimum interval one hour, daily run cap. **Connectors do NOT come along**: a routine created from inside a session carries none, and the parameter is refused there, so a routine that needs Gmail or Drive has to be created from the routines page on claude.ai. See section 10.
