...
- [x] ~~**Get the work onto `main`.**~~ **DONE 2026-08-24.** A session lands on its own `claude/...` branch by default, so the follow-up work was committed there first and then fast-forwarded onto `main`. No merge commit, no conflict. **`main` is the branch to read from.** If a future session lands on a `claude/...` branch, fast-forward `main` at the end rather than leaving the work stranded on the branch.
