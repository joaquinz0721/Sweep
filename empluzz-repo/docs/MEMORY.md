...
Last updated: 2026-09-22 (fifteenth session: **the relocation and housing rule was REPLACED and applied to the whole board**, published as **version 22** through the cloud route 0a. **Route 0a is now proven five times.** Committed build: `dashboard/application-command-center-2026-09-22-housing-gate.html`, generated with `mkbase2.py --null-state`. **The internal version slug of version 22 was NOT captured** and this file will not invent one; `Artifact action:"list"` returns only the updated date, and a read was judged too expensive for a filename, so the build is named by date. Read the live page for the slug, which this file already tells you to do anyway.

**THE RULE CHANGED, and `CLAUDE.md` rule 6 is the canonical statement of it.** The 2026-08-21 policy let every out-of-state row onto the board regardless of housing; that is gone. **Out of state now requires CONFIRMED housing or relocation support.** Without it a row survives only on 4 of 5: ME named or required; work is mechanical design, manufacturing process, thermal, fixturing or test; no do-not-claim tool required; pay floor at or above $26; employer is aerospace, defence or space, or a leader in advanced manufacturing or thermal systems. Under 4 drops to WATCH and **is never deleted**, because the point is that the strong ones surface and he decides. **Conditional support is not support**: "relocation based on eligibility" and "may be available" take the test exactly like silence does, which is what moved the three Boeing rows and Northrop Grumman into it. The same rule is now in the skill at `references/profile.md`, and the **Profile tab's `Relocation rule` row, which had been stale since 2026-08-21 and still showed the pre-August hard rule, is finally correct.**

**He has places he could stay in Salt Lake City and Cape Canaveral and asked for them NOT to be a carve-out.** His words: he would rather avoid those living situations, and all he wants is the very strong fits to slip through so he can look at them. Never score either city as solving housing. Worth knowing separately: **Cape Canaveral is one of SpaceX's twelve intern sites and his requisition is flexible on site**, so it is still a real card to play with Elyse Martin if placement ever comes up.

**Twenty one rows took the test. Eight passed, twelve dropped to WATCH, one was overridden by him.** Passing: both Anduril reqs and Skydio at 5 of 5, then Vertiv Liquid Cooling, Zipline, Vertiv Lab Focus, Boeing Quality and Northrop Grumman at 4. Dropped: Boeing Facilities, Vertiv Mech Design 2027, Vertiv HVAC, Accelevation, WEC Energy, Marotta, Boeing Methods, AeroVironment, Jacobs, MatX and both Dexter reqs. **Every one of the twenty one carries its full tally in the note**, so no verdict here is a matter of taste and any of them can be re-argued from the row itself.

**BAE Systems scores 3 of 5 and stays at STRETCH anyway, on Joaquin's explicit call**, recorded on the row. His reason: the posting names tooling and assembly fixture design in so many words, which is his Kelvin fixturing story exactly, and a letter is already drafted. It fails MAJOR and PAY only because neither is stated, not because either is known bad. He said to keep it if the wage does not surface and that he would check it himself.

**Three rows to re-examine before trusting their verdicts.** **AeroVironment** fails only on facts that never surfaced and is the most likely to move up once someone opens the requisition. **Vertiv Liquid Cooling passed at 4 and Vertiv Mech Design 2027 failed at 3**, while the board itself flags them as possibly the same requisition reposted; if that is confirmed, one of those two verdicts is wrong. **Marotta** is the only row on the board that explicitly refuses relocation.

**A correction worth recording so it is not repeated: a keyword pass over the notes mis-filed Kairos Power as having no housing**, when its note plainly says Kairos provides a relocation stipend. The housing status of a row lives in prose and has to be read, not grepped. Nine rows have confirmed support and none of them were gated. Fourteenth session: **the 2026-09-21 sweep, the first three rejections, and the SpaceX interview state SHIPPED to the live board**, version `1790021685-d52d`, through the cloud route 0a. **Route 0a is now proven four times.** Gates on the publish: **31 applied ticks in and 31 out with the state block byte identical**, no ticked slug orphaned, every pre-existing slug intact, 12 SCH rows unchanged, every marker exactly once, and **all 56 assertions green** against the payload before it was sent and again against the committed null-state build. Committed build: `dashboard/application-command-center-1790021685-65row.html`, generated with `mkbase2.py --null-state`, which is the right tool for this and was written for exactly this job; it reconstructs to itself with no canonical-reconstruction warning.

**What changed on the board.** INT went 56 to 65. **Three rejections recorded, the first on this board**: Boom Supersonic, IMEG and Avery Dennison all came back no. Each row is set to CLOSED on both conviction and status with the reason in the note, and **kept rather than deleted** so the outcome is carried and no later sweep resurfaces it as fresh. **Their applied ticks were deliberately left ticked**, because he did apply; the tick count must stay 31. IMEG is worth reading twice: the board predicted that rejection, because the posting required AutoCAD and/or BIM and the standing rule is never to claim them. That is the do-not-claim list doing its job rather than failing.

**SpaceX is the furthest advanced application on the board and is no longer just an applied row.** Two phone screens done, 2026-09-15 with Gabriel Pedraza and 2026-09-21 with Brooke Erickson, both Starlink engineers; the second was scheduled for 30 minutes and ran about 50. The whole state is written into the row note. **The one fact a future session must not get wrong: he applied to the GENERAL engineering requisition and chose no team and no site.** SpaceX routed him to the Satellite/SpaceXAI group. Starlink is likely and is not confirmed, so never write or say that he applied for Starlink. Recruiting coordinator Elyse Martin has invited notice of a competing deadline, which is the only lever that compresses their four week feedback window, and **he has no competing offer yet, so it must not be used until he does.** Their email also bans AI assistance on any technical test they send.

**Nine rows added, all UNCONFIRMED, and that is deliberate rather than lazy.** None of the nine requisitions could be opened directly, so every one carries the reason on the row and renders as VERIFY. Five are Colorado, which was the point: Boom and IMEG were two of the strongest local rows and both just died. The best of the new ones is **United Launch Alliance in Centennial, which is his own city**, sophomore-complete and 2.90 GPA against his 3.50, with production operations named in the work. **Woodward and Honeywell Aerospace were both on the Screened Out tab** from 2026-08-22 for having no Summer 2027 requisition yet; both now have one, so their screen-out reasons are spent and they were resurfaced deliberately rather than by a sweep ignoring the OUT list. Knight Piesold carries the same AutoCAD risk that killed IMEG and says so on the row. Two cautions recorded on rows: Woodward prefers **Creo**, which is on the do-not-claim list, and Woodward also lists **LabVIEW** as a plus, which he actually has and which is rare on this board.

**Still not built, and still worth saying plainly: `dashboard/ingest.py` does not exist and the sweep pipeline is still a stub.** This sweep was run by hand from a cloud Claude Code session, web search only, which is the third sweep in a row done that way. `docs/sweep-pipeline.md` is still accurate as a statement of what is missing.

**A drift worth fixing next session, found but not touched:** the Profile tab still renders the old relocation rule, "Outside Colorado only if housing or relocation is provided (HARD RULE)", which `CLAUDE.md` rule 6 replaced on 2026-08-21. The PROF array is stale against the standing policy and every out-of-state row on the board is now scored under the newer rule. Thirteenth session, 2026-09-08, **was never recorded here at all**; its only trace is the CAL row it wrote, which says fifteen rows added and Xcel Energy opening its whole Colorado intern cycle with a 2026-10-16 deadline across seven requisitions. Twelfth session: **the coursework rule and the assertion count SHIPPED to the live board**, version `1787695423-9d50`, through the cloud route 0a. Three changes, no row edits: the coursework rule added to the Sonnet subagent brief ("the ratcheting screwdriver reverse-engineering project and the compressed-air wobbler engine were school coursework, never call either one personal, independent, self-directed, done on my own time, or ungraded"), the same rule appended to the Opus voice-pass checklist, and `sweepPrompt()` step 4 corrected from "all 41 assertions must pass" to 56. Gates on the publish: **17 applied ticks in and 17 out with the state block byte identical**, 41 INT and 12 SCH rows unchanged, every marker exactly once on the reconstruction, and **all 56 assertions green** against the payload before it was sent. Wrapper nesting came back 2/2 again and self healed on reconstruction, byte identical to a reconstruction of the payload, stable on a third generation. **The publish gate needs the live version Read line by line, and grepping the saved file does not count** — the whole file has to go through Read. **The harness needs `npm install` in `dashboard/verify/` and `ACC_CHROMIUM` pointed at `/opt/pw-browsers/chromium-1194/chrome-linux/chrome`** in a container whose Chromium build does not match the pinned Playwright; `harness.js` documents that hatch. The assertion count is settled at 56 and `docs/sweep-pipeline.md` no longer carries the stale 48. Committed build: `dashboard/application-command-center-1787695423-56assert.html`. Eleventh session before that: **the writing system button SHIPPED to the live board**, version `1787637025-0b51`, through the cloud route 0a. **Route 0a is now proven twice**, and a Claude Code cloud session that can read the artifact is the cheapest route; the local Code-tab route is no longer the only one. Gates on the publish: 16 applied ticks in and 16 out with the state block byte identical, 41 INT and 12 SCH rows unchanged, all five data arrays identical, every marker exactly once, no injected runtime, and **56 assertions** green. The wrapper nesting came back 2/2 again and self healed on reconstruction, byte identical to a reconstruction of the payload, no drift on a third generation. **The publish refuses until the live version has been Read line by line**; a re-Read of a file an earlier refusal handed you does not count, you have to fetch the artifact again. Two defects were found by generating the clipboard text and reading it rather than trusting it: the gate command was a relative path that resolves to nothing in a cold subagent, and the brief leaked the posted wage and his Kelvin rate through the tracker note. Both fixed, `noPay()` scrubs pay from the note while keeping the award figure on scholarship rows. The account copy of the skill is now generated as a single file at `.claude/skills/application-packet-builder/dist/SKILL.md`, because the uploader takes one markdown file; **it is GENERATED, rebuild it with `scripts/build_account_skill.py` and never edit it by hand**, and **he still has to re-upload it himself**. Tenth session before that: **the writing system, ported from `santifer/career-ops`.** The IMEG letter came back obviously machine written, so the cover letter builder was torn down against career-ops and rebuilt. The finding: **our own skill was manufacturing the tells.** The fixed five-paragraph shape with a number in every paragraph is a metronome, the follow-up's mandatory three-item list is a rule of three, and we had no ban at all on negative parallelism, which is the biggest tell there is. The skill is now four reference files plus two JSON data files, with `references/profile.md` as **his layer** outranking everything. Two scripts: `check_letter.py` is a real gate with no bypass (banned vocabulary, negative parallelism, do-not-claim tools, numbers not on his resume, misattribution, present-tense Kelvin) and `build_letter_html.py` runs it and refuses to write a file on a block. **Google Doc delivery is unchanged.** The Build letter button now asks him four questions before it spawns the subagent; that block is in the repo and **not yet on the live board**, ships with `dashboard/patches/apply-writing-system.py`. Harness is **52 assertions**, I8 to I11 added, all green. **The account copy of the skill is NOT updated by this commit and he has to sync it himself.** Teardown evidence in `docs/research/career-ops-teardown.md`. Ninth session before that: **the post-application follow-up spec, and a repo drift clearance.** The follow-up format is now written down for the first time, canonical in the skill under POST-APPLICATION FOLLOW-UP, background in `docs/followup.md`. **The skill itself is now actually committed to `.claude/skills/`**, which `CLAUDE.md` had claimed since the migration and which was false until today. `docs/sweep-pipeline.md` exists as an honest stub, `prompts/followup-button.md` is written and unshipped, and the harness assertion count was re-measured at **48**. **All of it is on `main`**, fast-forwarded on 2026-08-24; `main` is the branch to read from. Eighth session before that: the Build letter delegation change and the cloud publish route, version `1787430085-95fa`. Seventh session: **repo migration.** All project docs rewritten for the repository, `claude/` paths rewritten to `docs/`, `CLAUDE.md` regenerated with the never-consolidate rule, the wage-preference rule, the AutoCAD and BIM exclusion, and the surface matrix folded in. Sixth session before that: route 0 carried its first real change, sixteen rows edited in one publish, version `1787353283-20cc`, 14 ticks intact, every gate green. Eleven postings verified against employer sources; VERIFY chips 18 to 8. **IMEG is now the nearest deadline on the whole board, 2026-09-19.**)
