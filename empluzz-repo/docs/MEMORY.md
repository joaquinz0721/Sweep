...
- **Repo versus live, checked 2026-08-22.** A cloud Claude Code session read the live artifact with the Artifact tool's `read` action, 117.5KB, version `1787428545-d4e8`. So reading from a cloud session works through that route, whatever WebFetch does. **Publishing from a cloud session is still untested and item 0 is still open.** The read showed the repo was two builds behind: the live board carries **41 internship rows and 14 ticks**, the file committed as `pre16row` carries 29 and a null state, and it is missing Boom Supersonic's verification among others. `dashboard/application-command-center-1787428545-41row.html` is the refreshed authored build, null state, patched with the new Build letter code, 48 assertions passing. **A republish must be built from a fresh read, not from that file, or the 14 ticks go to null.**
