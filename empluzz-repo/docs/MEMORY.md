...
- [x] ~~**Get the work onto `main`.**~~ **DONE 2026-08-24.** `main` had been stuck at `cd0c8a5 Add files via upload` since the initial upload while every real session committed to its own `claude/...` branch, so `main` was **seven commits behind** and carried none of the dashboard source, the harness, the delegation change, or the doc pass. It was a strict ancestor, so it fast-forwarded with no merge commit and no conflict. **Read from `main` from now on.** If a future session lands on a `claude/...` branch again, fast-forward `main` at the end rather than leaving the work stranded on the branch.
