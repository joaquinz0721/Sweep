...
- **Dashboard source:** `dashboard/application-command-center-1790116637-wagesplit.html`, the null-state copy of live version `1790116637-4820`, published 2026-09-22. It carries `applied:null`, so **never publish it as-is**; a republish is always built from a fresh read of the live page so his ticks survive. The verification harness is in `dashboard/verify/`, 56 assertions, `./run.sh`. One-change edit scripts live in `dashboard/patches/`. **The whole publish procedure, including the read the publish gate demands and what to do when it refuses twice, is `docs/artifact-publish-runbook.md`. Read it before touching the artifact.**
...
9. **The wage floor is a preference, not a filter.** `BASE_WAGE=26` is the Kelvin floor. He will still apply to roles under it as a last resort. Under-floor rows render red through the existing `.wg.down` style and are **never deleted and never downranked**. **Since 2026-09-22, on his instruction, they sit in their own "Pays under $26/hr" table under the main one on the Internships tab.** Only a range whose top end is under the floor moves; a straddling range or a row with no wage stays in the main table until he says otherwise. A wage derived from an annual figure carries an `est.` marker and the annual number lives in the note, not the wage field.
