...
7. **Numbers belong in resumes, cover letters, and personal messages, never on a public profile.** Yield figures and cycle times from a private employer's R&D do not go on an indexed page, so nothing goes in a LinkedIn headline, About section, post, or anything else the open web can read. **A direct message is not publication.** Settled by Joaquin 2026-08-22: figures are fine in a LinkedIn or Handshake message to one person, the same as in a cover letter. Keep it to one figure per message rather than three, since the recipient can forward it.
