...
6. **Relocation and housing, current policy as of 2026-09-22.** This REPLACES the 2026-08-21 policy, which let every out-of-state row onto the board regardless of housing. **Out of state now requires confirmed housing or relocation support by default.** A listing that does not confirm support belongs on the board only if the fit is exceptional, measured as **4 of these 5**: Mechanical Engineering is a named or required major; the work is mechanical design, manufacturing process, thermal, fixturing or test; no tool on the do-not-claim list is required; the posted pay floor is at or above $26; the employer is aerospace, defence or space, or a recognised leader in advanced manufacturing or thermal systems. **Under 4 drops to WATCH and is never deleted**, because the point is that the strong ones surface and he decides. **Every row that takes the test must carry its tally in the note**, so the verdict is checkable rather than a matter of taste. **Conditional support is not support**: relocation offered based on eligibility, or described as may be available, takes the test like silence does. The older distinction still holds and still matters: say plainly whether the listing is **SILENT** or **EXPLICITLY refuses**, because silence is not refusal. He has places he could stay in Salt Lake City and Cape Canaveral but would rather not use them, so they are **not** a carve-out and must never be scored as one. He may override any verdict on a row, and an override is recorded on that row with his reason.
